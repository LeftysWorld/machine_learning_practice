    ...
    def addEvidence(self, Xdata, Ydata):
        ...
        def build_tree(_Xdata, dataY):
            ...
            if len(set(_Xdata[:, feature])) == 1:
                ...
                SplitVal = get_split_val(_Xdata, feature)
                while SplitVal == np.amax(_Xdata[:, feature]):
                    SplitVal = get_split_val(_Xdata, feature)

                left_index, right_index = get_indices(_Xdata, feature, SplitVal)
                leftTree = build_tree(_Xdata[left_index], dataY[left_index])
                rightTree = build_tree(_Xdata[right_index], dataY[right_index])
                root = [feature, SplitVal, 1, len(leftTree) + 1]
