...
def get_best_allocations(syms, prices, rfr, sf):     # LOOK INTO THE LAMBDA FUNCTION USED IN THE CONSTRAINTS AND SPO.MINIMIZE
...
def get_daily_returns_data(prices, allocs):
    ...
    dr = reduce(lambda x, y: y(x), [lambda vals: get_portfolio_value_data(*vals), compute_daily_returns], [prices, allocs])
    return dr
