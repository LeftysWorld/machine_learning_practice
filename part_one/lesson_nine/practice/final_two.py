...
def determine_frequency_returns(freq, df):
    rates_to_function = {
        252.0: compute_daily_returns(df),
        52.0: compute_weekly_returns(df),
        12.0: compute_monthly_returns(df),
        1.0: compute_yearly_returns(df)
    }
    return rates_to_function[freq] if freq in rates_to_function else compute_daily_returns(df)
...
def get_portfolio_value(prices, allocations, sv):       # Still need to figure out if I need to put in the initial investment data into this
    """Get portfolio value"""
...
def assess_portfolio(sd=dt.datetime(2008, 1, 1), ed=dt.datetime(2009, 1, 1),
    ...
    portfolio_returns = determine_frequency_returns(freq, portfolio_value)
...
def test_run():
    ...
    allocations = [0.0, 0.9, 0.1, 0.0]
    start_val = 1000000
    risk_free_rate = 0.0
    sample_freq = 252

    """
    could nto test risk free rate if not 0
    """
